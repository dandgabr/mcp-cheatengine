...
Ele permite que agentes de IA realizem análises de memória, engenharia reversa, busca por assinaturas AOB, navegação por cadeias de ponteiros, desensamblagem x86/x64, injeção de código Auto Assemble, controle de execução/speedhack e automação Lua via comandos em linguagem natural com **validação estrita de segurança e isolamento por sandbox**.
...
```mermaid
flowchart LR
    AI[Assistente de IA\nClaude / Antigravity / Cursor] <-->|MCP Stdio Protocol| PyServer[Servidor Python MCP\nsrc/mcp_cheatengine/server.py]
...
```
mcp-cheatengine/
├── .gitignore                      # Configuração completa do GitIgnore (Python & Lua)
├── LICENSE                         # Licença MIT
├── README.md                       # Documentação principal
├── requirements.txt                # Dependências Python (mcp)
├── pyproject.toml                  # Configuração de empacotamento Python
├── ce_mcp_server.py                # Wrapper de compatibilidade raiz
...
## 🛠️ Tabela das 21 Ferramentas MCP
...
Todas as 21 ferramentas passaram por auditoria estrita de segurança via agente subespecializado (`security_validator`):
